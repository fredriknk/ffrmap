...
var breadcrumbs = [];
var standstill = 0;
var stationaryStart = null; // When the robot becomes stationary
var stationary_warning = 240;
var currentColoringMode = 'time'; // Default coloring mode


standstill_warning = 8; // Number of seconds to wait before displaying warning
...
function shouldAddBreadcrumb(latestData) {
    ...
    console.log(latestData.latitude, latestData.longitude, distance, breadcrumbs.length);
    return distance >= 0.3;
}

function incrementStandstill() {
    if (standstill === 0) {
        stationaryStart = new Date();
    }
    standstill++;
    if (standstill >= standstill_warning) {
        displayWarning();
    }
}

function resetStandstill() {
    standstill = 0;
...
const colorMappings = {
    ...
    satellite: {
        '> 30 Satellites': '#00ff00',
        '15-30 Satellites': '#ffff00',
        '< 15 Satellites': '#ff0000'
    }
};
...
// Start the periodic update of the map
